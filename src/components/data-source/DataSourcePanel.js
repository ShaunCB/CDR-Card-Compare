...
class DataSourcePanel extends React.Component {
  ...
  render() {
    ...
    const updateVersions = () => {
      if (xV && xMinV && (xV !== vHeaders.xV || xMinV !== vHeaders.xMinV)) {
        ...
        dataSources.forEach((dataSource, dataSourceIndex) => {
          const isBankingOrLending = !dataSource.deleted && (!dataSource.sectors || dataSource.sectors.includes("banking") || dataSource.sectors.includes("non-bank-lending"))
          if (!dataSource.unsaved && !dataSource.deleted && dataSource.enabled && isBankingOrLending) {
    ...
    const isBankingOrLending = dataSource => {
      return !dataSource.deleted && (!dataSource.sectors || dataSource.sectors.includes("banking") || dataSource.sectors.includes("non-bank-lending"))
    }
      ...
      <Accordion defaultExpanded={false} className={classes.panel}>
        ...
        >
          <div className={classes.heading}>
            <StorageIcon/><Typography style={{paddingLeft: 8}}>Data sources</Typography>
          </div>
        </AccordionSummary>
        <div style={{maxHeight: 300, overflow: 'auto'}}>
          ...
          <div className={classes.details}>
            ...
            {dataSources.map((dataSource, index) =>
              isBankingOrLending(dataSource) && <DataSource key={index} dataSource={dataSource} index={index}/>)}
          </div>
          }
        </div>
        <Divider/>
