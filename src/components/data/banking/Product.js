...
import { normalise, sanitizeUrl } from '../../../utils/url'
import { retrieveProductDetail } from '../../../store/banking/data'
...
const Product = (props) => {
  ...
  const {product, dataSourceIndex, selectedProducts, dataSource, versionInfo} = props
  ...
  const handleChange = event => {
    if (event.target.checked) {
      ...
      if (dataSource) {
        ...
        props.retrieveProductDetail(dataSourceIndex, normalisedUrl, product.productId, versionInfo.xV, versionInfo.xMinV)
      }
    } else {
  ...
  return (
    ...
    <Accordion defaultExpanded={false}>
      ...
      <div style={{fontSize: '0.8rem'}}>
        ...
        {!!product.applicationUri && !!sanitizeUrl(product.applicationUri) && <div><a href={sanitizeUrl(product.applicationUri)} target='_blank' rel='noopener noreferrer'>View Issuer Website</a></div>}
        {
          ...
          <div>
            ...
            <ul className={classes.sectionContent}>
              {product.cardArt.map((cardArt, index) =>
                <CardArt key={index} cardArt={cardArt} dataSource={dataSource}/>
              )}
            </ul>
          </div>
        }
...
const mapStateToProps = (state, ownProps) => ({
  ...
  dataSource: state.dataSources[ownProps.dataSourceIndex],
  versionInfo: state.versionInfo.vHeaders
})

const mapDispatchToProps = { selectProduct, deselectProduct, retrieveProductDetail }
